...
export function Contact({ text = "Duvidas? Fale conosco" }) {
  return <a href={`https://wa.me/5511945316242?text=Oii%2C%20é%20sobre%20o%20Droplinkfy%2C%20você%20consegue%20me%20ajudar%3F`} className="flex text-[#5e92fa] items-center !text-center justify-center font-light mt-auto border-0 py-3 px-4 w-full transition-all">
...
function Button({ text = "Acessar Agora", onClick = evt => { } }) {
  return <button onClick={onClick} className="flex items-center !text-center justify-center uppercase font-bold mt-auto text-white bg-[#e5ac31] border-0 py-3 px-4 w-full focus:outline-none hover:bg-[#eabd5a] rounded-full shadow-lg hover:shadow-xl transition-all">
    {text}
  </button>
}
